...
- Light on/of (relay)
- LED (pwm)
- Switch click, douledclick, hold : HW switch (GND & PIN)
- Temperature & Humidity (HDT11)
- Light intensity
- Alarm (PIR motion)

- compatible with Apple HOMEKIT
    look at >>__________--.go<<

  REST API:
  - PINS    xx.xx.xx.xx/pins/4 (GET)
  - PINS xx.xx.xx.xx/pins/4 (POST : on/off/pwm)
  - PINS config xx.xx.xx.xx/pins/4/config (GET / POST)
  - device xx.xx.xx.xx/device (GET / POST)
...
//example: http://arduino-esp8266.readthedocs.io/en/latest/esp8266wifi/server-examples.html
//doc: http://arduino-esp8266.readthedocs.io/en/latest/esp8266wifi/server-class.html
...
bool digitalWritePins[] = { false, false, false, false, false, true, true, true, true};   //piny 5, 6, 7, 8 jsou vystupní
bool relayOnHighPins[] = { true, true, true, true, true, true, true, true, true}; //čím se sepne relé true = HIGH
bool inusePin[] = { true, false, true, true, true, true, true, false, false};       //jeli pin používán
...
void handleRoot() {         //help v rootu IP adresy
  ...
  message += "<a href=\"http://\\" + String(deviceId) + ".local/pins/4 \">http://" +  String(deviceId) + ".local/pins/4</a></p>";
  ...
  message += "<a href=\"http://\\" + String(deviceId) + ".local/pins/4/config \">http://" +  String(deviceId) + ".local/pins/4/config</a></p>";
  ...
  message += "<a href=\"http://\\" + String(deviceId) + ".local/device \"> http://" +  String(deviceId) + ".local/device</a></p>";
  ...
  message += "<a href=\"http://\\" + String(deviceId) + ".local/analog \"> http://" +  String(deviceId) + ".local/analog</a></p>";

  message += "<br><p>PIR: (je jen pinu 2)";
  message += "<a href=\"http://\\" + String(deviceId) + ".local/pir/2 \"> http://" +  String(deviceId) + ".local/analog</a></p>";
...
void setup(void) {
  ...
  server.on("/pins/7", HTTP_GET, handleGetDigiPin);
  ...
  server.on("/pins/7", HTTP_POST, handleSetDigiPin);
  ...
  server.on("/pins/7", HTTP_OPTIONS, handleOptionsDigiPin);
